...
ecds_events_df <- dbGetQuery(
  ...
  inner_join(model_provider_sample_df,
                    join_by(procode)) |> 
  filter(dttm_depart > as_datetime("2022-03-31 23:59:59") | is.na(dttm_depart)) |> 
  filter(dttm_arr < as_datetime("2024-08-01 00:00:00")) 
...
saveRDS(ecds_events_df, here("data", "ecds_events_df.RDS"))
